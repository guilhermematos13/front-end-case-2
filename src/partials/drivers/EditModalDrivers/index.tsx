...
export function EditModalDrivers({
  handleCloseModal,
  openModal,
  fetchDriver,
  idDriver,
  ...
  useEffect(() => {
    openModal &&
      ...
      api.get(`condutor/${idDriver}`).then((response) => {
        ...
        setValue(
          'driverDocument',
          response.data.catergoriaHabilitacao.split(','),
        )
        setValue(
          'licenseExpirationDate',
          new Date(response.data.vencimentoHabilitacao),
        )
      })
  }, [setValue, openModal, idDriver])
